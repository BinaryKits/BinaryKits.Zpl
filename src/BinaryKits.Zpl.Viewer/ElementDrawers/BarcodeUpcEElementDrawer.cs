        ...
        public override SKPoint Draw(ZplElementBase element, SKCanvas canvas, IPrinterStorage printerStorage, DrawerOptions options, SKPoint currentPosition, InternationalFont internationalFont, int printDensityDpmm)
        {
            if (element is ZplBarcodeUpcE barcode)
            {
                ...
                if (barcode.PrintInterpretationLine)
                {
                    ...
                    {
                        this.DrawUpcEInterpretationLine(result, interpretation, canvas, labelFont, x, y, resizedImage.Width, resizedImage.Height, barcode.FieldOrigin != null, barcode.FieldOrientation, barcode.ModuleWidth, options);
                    }
                }
